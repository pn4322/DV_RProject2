...
ggplot() + 
  ...
  labs(title='NewYork') +
  labs(x="Ethnicity", y=paste("Year")) +
  layer(data=df, 
        mapping=aes(x=(as.character(SEX)), y=as.numeric(as.character(YEAR_)), color=SEX), 
        stat="identity", 
        stat_params=list(), 
        geom="jitter",
        geom_params=list(), 
